...
class TestFile():
		...
		if self.lostflg == 1:
			if str1[0:1]  == "[":
				for i in range(0,len(str1)/15):
					str2 = str1[i*15:15+i*15]
					self.lostuid = self.lostuid + str2[1:4] + ", "
					#self.lostuid = self.lostuid + "%string.atoi(str2[1:4])s" % str2[1:4] + ", "
		if str1[0:3]  == "ok:":
			if self.sumcount != self.max:
				self.lostflg = 0
	...
	def result_analysis(self,str):
		...
		if str1[0:3] == "Use":
			self.usetime = str1[11:]

		if str[0:3]  == "Sum":
			self.str ="<%03d>" % self.lineindex + "[%9s]" % self.usetime + self.str + " Sum:" + str1[10:13] +","
